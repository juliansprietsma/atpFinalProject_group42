globals [
  num-shallow-supers
  spacer-size
  strive-food-level
  sight-radius
  honey-max
  storage-threshold ;;the amount of food a bee needs to carry before it stores
  forage-threshold
  total-honey
  hunger-threshold
  feeding-cooldown
]

breed [workers a-worker]
breed [drones a-drone]

workers-own [
age ;; age of the bee
phoretic-mite-count ;does the bee have a mite attached
food ;;food level
designation ;;hive bee or forager
health ;; health of the bee
foraging;;
activity
]
drones-own [
age
health ;; health of the bee
phoretic-mite-count
]

patches-own [
 growth-stage
 mite-presence
 has-brood
 brood-cell
 honey-cell
 honey-level
 food-level
 larvae-health
 larvae-type
 cooldown
]

to set-globals ;;inspired by lecture code
  set spacer-size 4
  set strive-food-level 80
  set sight-radius 4
  set honey-max 200
  set storage-threshold 150
  set forage-threshold 200
  set hunger-threshold 70
  set feeding-cooldown 50
end

to update-globals ;;code from simple seggregation model 2006 Uri Wilensky.
  ;;let similar-neighbors sum [similar-nearby] of turtles
  ;;let total-neighbors sum [total-nearby] of turtles
  let honey-cells count patches with [honey-cell = true]
  let honey sum [honey-level] of patches with [honey-cell = true]

  set total-honey honey
end

to setup
  clear-all
  set-globals
  generate-beehive
  reset-ticks
end

to go
  update-globals
  brood-development

  ask turtles [
   set age age + 1



  ]

   ask turtles with [breed = workers][
   ifelse(food <= 0)[
     die
    ][
     if random 4 = 1 [
       set food food - 1
      ]

    ]

   if phoretic-mite-count > 0 [
     mite-jump

    ]

   if(designation = "hiver")[

   withdraw-honey
   feed-larvae

   ifelse random 10 = 1[
    wiggle
      ][
        move-forward]
    ]
   if(designation = "forager")[
   deposit-honey
   withdraw-honey

   ifelse(forage-threshold > food)[
     set foraging true
     setxy min-pxcor min-pycor
     forage
      ][
      if(foraging = true)[
         set foraging false
         setxy random-pxcor (min-pycor + bottom-board-size) + random (max-pycor - (min-pycor + bottom-board-size)+ 1)
        ]
        move-forward
      ]
    ]
   if (age > 1000) [
     set designation "forager"
    ]

   if(age > 4000 * (health / 100))[
   die
  ]
  ]

  ask turtles with [breed = drones][
   move-forward
       if(age > 2000 * (health / 100))[
   die
  ]
  ]


  tick ;; advance the clock by one “tick”
end


to brood-development
 ask patches with [has-brood = true][

    if(cooldown > 0)[
     set cooldown cooldown - 1
    ]

    set pcolor scale-color yellow growth-stage 201 1
    ;;This code is incomplete, growth needs to depend on worker involement
    set growth-stage growth-stage + 1
    if(growth-stage < 90)[ ;;sort of random value right now, but this represents when the cell is capped and the brood grows on its own
        if food-level > 0[set food-level food-level - 1 ]
       if food-level <= 0[ ;;The larvae dies if there is no more food
        set pcolor yellow - 0.5
        set growth-stage 0
        set has-brood false
        set mite-presence false
       ]
    ]

    if growth-stage > 200[
      set pcolor yellow - 0.5
      set growth-stage 0
      set has-brood false
      let temp-mite-presence mite-presence
      if larvae-type = "worker"[
      sprout-workers 1 [
          set color green
          set shape "bug"
          set size 3
          set age 0
          set food 100
          ifelse(temp-mite-presence = true)[
            set health random 60 ;;needs something bett
            set phoretic-mite-count 2 ;;tbd later
          ][
           set health 100
          ]
          set designation "hiver"


      ] ]
      if larvae-type = "drone"[
        sprout-drones 1 [
        set color green
        set shape "bug"
        set size 3
        set age 0
        ifelse(temp-mite-presence = true)[
            set health random 60 ;;needs something bett
            set phoretic-mite-count 3 ;;tbd later
          ][
            set health 100
          ]
        ]
      ]
      set mite-presence false
    ]
  ]
  if ticks mod egg-time = 0 [

    if any? patches with [brood-cell = true and has-brood = false][
    ask n-of 3 patches with [brood-cell = true and has-brood = false] [
     set has-brood true
     set growth-stage 1
     set food-level 50
     set larvae-health 100
     ifelse random 20 = 1[ ;;may need to add global variable to control drone %
     set larvae-type "drone"
        ][
     set larvae-type "worker"
        ]
    ]
    ]

  ]



end

to generate-beehive

 let x-max 150
 let y-max ((bottom-board-size + deep-super-size + shallow-super-size) / 2)
 let y-min y-max * (-1)
 let x-min x-max * (-1)

 resize-world x-min x-max y-min y-max

 ask patches [
    if pycor > min-pycor + bottom-board-size [
      set has-brood false
        set brood-cell true
        set honey-cell  false
        set pcolor yellow - 0.5
        set honey-cell true

    ]
    if (pycor = (min-pycor + bottom-board-size))[
      set pcolor red
    ]

  ]
    ;;setup bees
     create-workers num-workers [
    set color green
    set shape "bug"
    set size 3
    set age random 1800
    set food 400
    set health 100

    if(setup-mites = true and random 10 = 1)[
     set phoretic-mite-count 1 ;;infect some workers with mites from the start
    ]
    ifelse random 3 = 1[ ;;may need to add global variable to control forager %
      set designation "forager"
  ][
     set designation "hiver"
    ]

    setxy random-pxcor (min-pycor + bottom-board-size) + random (max-pycor - (min-pycor + bottom-board-size)+ 1) ; Place them randomly but above the bottom board
  ]
  create-drones 1 [ ;;will change
    set color green
    set shape "bug"
    set size 3
    set age 0
    set health 100
    setxy random-pxcor (min-pycor + bottom-board-size) + random (max-pycor - (min-pycor + bottom-board-size)+ 1)
  ]

end


to wiggle
  rt random 90
  lt random 90
end

to move-forward
  ifelse patch-ahead 1 != nobody [ ;;To use the patch ahead function we need to deal with edge of world.
  ifelse not ([pcolor] of patch-ahead 1 = red)[
        forward 1 * (health / 100)][
   wiggle
  ]
  ][
   wiggle
  ]
end





to move-forage
  ifelse patch-ahead 1 != nobody [
    forward 1 * (health / 100)
  ][
   wiggle
  ]
end


to deposit-honey

if food > storage-threshold[


let relevant-patches patches in-radius sight-radius with [honey-cell = true and honey-max > honey-level]

  if any? relevant-patches[

    let temp-food food
    ask min-n-of 1 relevant-patches [honey-level][

    ifelse ((temp-food - storage-threshold) + honey-level) > honey-max[
      set temp-food temp-food - (honey-max - honey-level)
      set honey-level honey-max
      ][
       set honey-level honey-level + (temp-food - storage-threshold)
       set temp-food storage-threshold

        ]

    ]
      set food temp-food
    ]


  ]

end

to withdraw-honey

if storage-threshold > food[

let relevant-patches patches in-radius sight-radius with [honey-cell = true and honey-level > 0]

  if any? relevant-patches[

    let temp-food food
    ask max-n-of 1 relevant-patches [honey-level][

    ifelse(honey-level > 0.5 * honey-max)[ ;;Arbitrary percentage, global variable, or too trivial?
    ifelse (honey-level > (storage-threshold - temp-food)) [

      set honey-level (honey-level - (storage-threshold - temp-food))
      set temp-food storage-threshold
      ][

       set temp-food temp-food + honey-level
       set honey-level 0
        ]
        ][
         if(hunger-threshold > temp-food)[
            ifelse (honey-level > (hunger-threshold - temp-food)) [
              set honey-level (honey-level - (hunger-threshold - temp-food))
              set temp-food hunger-threshold
            ][

              set temp-food temp-food + honey-level
              set honey-level 0
            ]

          ]

        ]

    ]
      set food temp-food
    ]


  ]


end

to mite-jump ;;behavior for mites jumping from bees to brood
 ;; let drone-patches patches in-radius 2 with [has-brood = true and 10 > growth-stage and larvae-type = "drone"]
  ;;let worker-patches patches in-radius 2 with [has-brood = true and 10 > growth-stage and larvae-type = "worker"]
  ;;imperfect probabilities but they are fine enough like this
  let brood-patches patches in-radius 5 with [has-brood = true and 50 > growth-stage and mite-presence != true ]
  let free-bees turtles in-radius 5 with [phoretic-mite-count = 0 and age < 200]


  if any? brood-patches[
   set phoretic-mite-count phoretic-mite-count - 1
   ask one-of brood-patches[

     ;;Drone preference?
     set mite-presence true
    ]
  ]

  if phoretic-mite-count > 0[
  if any? free-bees[
   set phoretic-mite-count phoretic-mite-count - 1
   ask one-of free-bees[
        set phoretic-mite-count 1
    ]
  ]
  ]




 ; ifelse random 2 = 1 [
  ;  if any? drone-patches[
   ; set phoretic-mite-count phoretic-mite-count - 1
    ; ask n-of 1 drone-patches[
    ;    set mite-presence true
    ;  ]
    ;]
 ; ][
  ; if random 2 = 1[
   ; if any? worker-patches[
    ;set phoretic-mite-count phoretic-mite-count - 1
    ; ask n-of 1 worker-patches[
     ;   set mite-presence true
      ;]
  ;  ]
  ;  ]
 ; ]


end

to forage
  ;ifelse(ycor > ((-1)*((bottom-board-size + deep-super-size + shallow-super-size) / 2) + 5))[;;if the bee is not at the exit make it go there
   ; set heading 180
   ; move-forage
  ;][

  ifelse winter = false [
 if(random ((100 - health)) = 1)[ ;;foraging is less successful if the bee has bad health
     set food food + 1
     ;;set health health - 1 ;;foraging is dangerous
  ]][
     if(random (4 + (100 - health)) = 1)[ ;;foraging is less successful if the bee has bad health
     set food food + 1
     ;;set health health - 1 ;;foraging is dangerous
  ]]


  ;]



end

to social-heading ;;adjust heading to be more social and care for brood
  let brood-patches patches in-radius sight-radius with [has-brood = true and 90 > growth-stage and distance myself > 1]
  ifelse (random 4 = 1)[ ;;adjust heading every once in while
  if any? brood-patches[
      set heading (towards (min-one-of brood-patches [distance myself]))
  ]
  ][
    wiggle

  ]

end

to feed-larvae

  let relevant-patches patches in-radius sight-radius with [has-brood = true and growth-stage < 90 and (food-level < strive-food-level) and cooldown = 0]

  if any? relevant-patches[

    let temp-food food
    ask min-n-of 1 relevant-patches [food-level][
  set cooldown feeding-cooldown
      ifelse(temp-food > (strive-food-level - food-level))[
        set temp-food temp-food - (strive-food-level - food-level)
        set food-level strive-food-level
      ][

        set food-level food-level + temp-food
        set temp-food 0

  ]
  ]

  set food temp-food
  ]

end

@#$#@#$#@
GRAPHICS-WINDOW
200
10
1412
615
-1
-1
4.0
1
10
1
1
1
0
0
0
1
-150
150
-74
74
0
0
1
ticks
30.0

BUTTON
103
165
172
201
go
go
T
1
T
OBSERVER
NIL
NIL
NIL
NIL
0

BUTTON
13
166
83
202
setup
setup
NIL
1
T
OBSERVER
NIL
NIL
NIL
NIL
1

SLIDER
7
474
179
507
num-workers
num-workers
1
3000
2179.0
1
1
NIL
HORIZONTAL

SLIDER
9
529
182
562
bottom-board-size
bottom-board-size
1
30
21.0
1
1
NIL
HORIZONTAL

SLIDER
9
61
182
94
deep-super-size
deep-super-size
0
100
89.0
1
1
NIL
HORIZONTAL

SLIDER
8
10
181
43
shallow-super-size
shallow-super-size
0
100
39.0
1
1
NIL
HORIZONTAL

SLIDER
14
665
186
698
egg-time
egg-time
1
250
6.0
1
1
NIL
HORIZONTAL

PLOT
1442
47
1642
197
Worker bee count
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [breed = workers]"

MONITOR
1468
390
1683
435
NIL
count turtles with [breed = workers]
17
1
11

PLOT
1658
50
1858
200
foragers
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count turtles with [breed = workers and designation = \"forager\"]"

PLOT
1658
223
1858
373
Honey
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot total-honey"

PLOT
1434
227
1634
377
Total capped brood
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [growth-stage > 90]"

PLOT
1463
455
1663
605
Phoretic mites
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot sum [phoretic-mite-count] of turtles"

PLOT
1672
453
1872
603
Brood mites
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [mite-presence = true]"

SWITCH
32
121
152
154
setup-mites
setup-mites
1
1
-1000

PLOT
1228
465
1428
615
brood
NIL
NIL
0.0
10.0
0.0
10.0
true
false
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count patches with [has-brood = true]"

SWITCH
20
222
123
255
winter
winter
0
1
-1000

@#$#@#$#@
@#$#@#$#@
default
true
0
Polygon -7500403 true true 150 5 40 250 150 205 260 250

airplane
true
0
Polygon -7500403 true true 150 0 135 15 120 60 120 105 15 165 15 195 120 180 135 240 105 270 120 285 150 270 180 285 210 270 165 240 180 180 285 195 285 165 180 105 180 60 165 15

arrow
true
0
Polygon -7500403 true true 150 0 0 150 105 150 105 293 195 293 195 150 300 150

box
false
0
Polygon -7500403 true true 150 285 285 225 285 75 150 135
Polygon -7500403 true true 150 135 15 75 150 15 285 75
Polygon -7500403 true true 15 75 15 225 150 285 150 135
Line -16777216 false 150 285 150 135
Line -16777216 false 150 135 15 75
Line -16777216 false 150 135 285 75

bug
true
0
Circle -7500403 true true 96 182 108
Circle -7500403 true true 110 127 80
Circle -7500403 true true 110 75 80
Line -7500403 true 150 100 80 30
Line -7500403 true 150 100 220 30

butterfly
true
0
Polygon -7500403 true true 150 165 209 199 225 225 225 255 195 270 165 255 150 240
Polygon -7500403 true true 150 165 89 198 75 225 75 255 105 270 135 255 150 240
Polygon -7500403 true true 139 148 100 105 55 90 25 90 10 105 10 135 25 180 40 195 85 194 139 163
Polygon -7500403 true true 162 150 200 105 245 90 275 90 290 105 290 135 275 180 260 195 215 195 162 165
Polygon -16777216 true false 150 255 135 225 120 150 135 120 150 105 165 120 180 150 165 225
Circle -16777216 true false 135 90 30
Line -16777216 false 150 105 195 60
Line -16777216 false 150 105 105 60

car
false
0
Polygon -7500403 true true 300 180 279 164 261 144 240 135 226 132 213 106 203 84 185 63 159 50 135 50 75 60 0 150 0 165 0 225 300 225 300 180
Circle -16777216 true false 180 180 90
Circle -16777216 true false 30 180 90
Polygon -16777216 true false 162 80 132 78 134 135 209 135 194 105 189 96 180 89
Circle -7500403 true true 47 195 58
Circle -7500403 true true 195 195 58

circle
false
0
Circle -7500403 true true 0 0 300

circle 2
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240

cow
false
0
Polygon -7500403 true true 200 193 197 249 179 249 177 196 166 187 140 189 93 191 78 179 72 211 49 209 48 181 37 149 25 120 25 89 45 72 103 84 179 75 198 76 252 64 272 81 293 103 285 121 255 121 242 118 224 167
Polygon -7500403 true true 73 210 86 251 62 249 48 208
Polygon -7500403 true true 25 114 16 195 9 204 23 213 25 200 39 123

cylinder
false
0
Circle -7500403 true true 0 0 300

dot
false
0
Circle -7500403 true true 90 90 120

face happy
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 255 90 239 62 213 47 191 67 179 90 203 109 218 150 225 192 218 210 203 227 181 251 194 236 217 212 240

face neutral
false
0
Circle -7500403 true true 8 7 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Rectangle -16777216 true false 60 195 240 225

face sad
false
0
Circle -7500403 true true 8 8 285
Circle -16777216 true false 60 75 60
Circle -16777216 true false 180 75 60
Polygon -16777216 true false 150 168 90 184 62 210 47 232 67 244 90 220 109 205 150 198 192 205 210 220 227 242 251 229 236 206 212 183

fish
false
0
Polygon -1 true false 44 131 21 87 15 86 0 120 15 150 0 180 13 214 20 212 45 166
Polygon -1 true false 135 195 119 235 95 218 76 210 46 204 60 165
Polygon -1 true false 75 45 83 77 71 103 86 114 166 78 135 60
Polygon -7500403 true true 30 136 151 77 226 81 280 119 292 146 292 160 287 170 270 195 195 210 151 212 30 166
Circle -16777216 true false 215 106 30

flag
false
0
Rectangle -7500403 true true 60 15 75 300
Polygon -7500403 true true 90 150 270 90 90 30
Line -7500403 true 75 135 90 135
Line -7500403 true 75 45 90 45

flower
false
0
Polygon -10899396 true false 135 120 165 165 180 210 180 240 150 300 165 300 195 240 195 195 165 135
Circle -7500403 true true 85 132 38
Circle -7500403 true true 130 147 38
Circle -7500403 true true 192 85 38
Circle -7500403 true true 85 40 38
Circle -7500403 true true 177 40 38
Circle -7500403 true true 177 132 38
Circle -7500403 true true 70 85 38
Circle -7500403 true true 130 25 38
Circle -7500403 true true 96 51 108
Circle -16777216 true false 113 68 74
Polygon -10899396 true false 189 233 219 188 249 173 279 188 234 218
Polygon -10899396 true false 180 255 150 210 105 210 75 240 135 240

house
false
0
Rectangle -7500403 true true 45 120 255 285
Rectangle -16777216 true false 120 210 180 285
Polygon -7500403 true true 15 120 150 15 285 120
Line -16777216 false 30 120 270 120

leaf
false
0
Polygon -7500403 true true 150 210 135 195 120 210 60 210 30 195 60 180 60 165 15 135 30 120 15 105 40 104 45 90 60 90 90 105 105 120 120 120 105 60 120 60 135 30 150 15 165 30 180 60 195 60 180 120 195 120 210 105 240 90 255 90 263 104 285 105 270 120 285 135 240 165 240 180 270 195 240 210 180 210 165 195
Polygon -7500403 true true 135 195 135 240 120 255 105 255 105 285 135 285 165 240 165 195

line
true
0
Line -7500403 true 150 0 150 300

line half
true
0
Line -7500403 true 150 0 150 150

pentagon
false
0
Polygon -7500403 true true 150 15 15 120 60 285 240 285 285 120

person
false
0
Circle -7500403 true true 110 5 80
Polygon -7500403 true true 105 90 120 195 90 285 105 300 135 300 150 225 165 300 195 300 210 285 180 195 195 90
Rectangle -7500403 true true 127 79 172 94
Polygon -7500403 true true 195 90 240 150 225 180 165 105
Polygon -7500403 true true 105 90 60 150 75 180 135 105

plant
false
0
Rectangle -7500403 true true 135 90 165 300
Polygon -7500403 true true 135 255 90 210 45 195 75 255 135 285
Polygon -7500403 true true 165 255 210 210 255 195 225 255 165 285
Polygon -7500403 true true 135 180 90 135 45 120 75 180 135 210
Polygon -7500403 true true 165 180 165 210 225 180 255 120 210 135
Polygon -7500403 true true 135 105 90 60 45 45 75 105 135 135
Polygon -7500403 true true 165 105 165 135 225 105 255 45 210 60
Polygon -7500403 true true 135 90 120 45 150 15 180 45 165 90

square
false
0
Rectangle -7500403 true true 30 30 270 270

square 2
false
0
Rectangle -7500403 true true 30 30 270 270
Rectangle -16777216 true false 60 60 240 240

star
false
0
Polygon -7500403 true true 151 1 185 108 298 108 207 175 242 282 151 216 59 282 94 175 3 108 116 108

target
false
0
Circle -7500403 true true 0 0 300
Circle -16777216 true false 30 30 240
Circle -7500403 true true 60 60 180
Circle -16777216 true false 90 90 120
Circle -7500403 true true 120 120 60

tree
false
0
Circle -7500403 true true 118 3 94
Rectangle -6459832 true false 120 195 180 300
Circle -7500403 true true 65 21 108
Circle -7500403 true true 116 41 127
Circle -7500403 true true 45 90 120
Circle -7500403 true true 104 74 152

triangle
false
0
Polygon -7500403 true true 150 30 15 255 285 255

triangle 2
false
0
Polygon -7500403 true true 150 30 15 255 285 255
Polygon -16777216 true false 151 99 225 223 75 224

truck
false
0
Rectangle -7500403 true true 4 45 195 187
Polygon -7500403 true true 296 193 296 150 259 134 244 104 208 104 207 194
Rectangle -1 true false 195 60 195 105
Polygon -16777216 true false 238 112 252 141 219 141 218 112
Circle -16777216 true false 234 174 42
Rectangle -7500403 true true 181 185 214 194
Circle -16777216 true false 144 174 42
Circle -16777216 true false 24 174 42
Circle -7500403 false true 24 174 42
Circle -7500403 false true 144 174 42
Circle -7500403 false true 234 174 42

turtle
true
0
Polygon -10899396 true false 215 204 240 233 246 254 228 266 215 252 193 210
Polygon -10899396 true false 195 90 225 75 245 75 260 89 269 108 261 124 240 105 225 105 210 105
Polygon -10899396 true false 105 90 75 75 55 75 40 89 31 108 39 124 60 105 75 105 90 105
Polygon -10899396 true false 132 85 134 64 107 51 108 17 150 2 192 18 192 52 169 65 172 87
Polygon -10899396 true false 85 204 60 233 54 254 72 266 85 252 107 210
Polygon -7500403 true true 119 75 179 75 209 101 224 135 220 225 175 261 128 261 81 224 74 135 88 99

wheel
false
0
Circle -7500403 true true 3 3 294
Circle -16777216 true false 30 30 240
Line -7500403 true 150 285 150 15
Line -7500403 true 15 150 285 150
Circle -7500403 true true 120 120 60
Line -7500403 true 216 40 79 269
Line -7500403 true 40 84 269 221
Line -7500403 true 40 216 269 79
Line -7500403 true 84 40 221 269

x
false
0
Polygon -7500403 true true 270 75 225 30 30 225 75 270
Polygon -7500403 true true 30 75 75 30 270 225 225 270
@#$#@#$#@
NetLogo 6.4.0
@#$#@#$#@
set density 60.0
setup
repeat 180 [ go ]
@#$#@#$#@
@#$#@#$#@
<experiments>
  <experiment name="experiment-tens-moore" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>((count patches with [shade-of? pcolor red] - initial-burnt) / initial-trees   * 100)</metric>
    <enumeratedValueSet variable="density">
      <value value="30"/>
      <value value="40"/>
      <value value="50"/>
      <value value="60"/>
      <value value="70"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="N">
      <value value="50"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="Neighborhood">
      <value value="&quot;Moore&quot;"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="burn-one">
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="fire-stop-grid">
      <value value="false"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="experiment-grid" repetitions="25" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>(count patches with [shade-of? pcolor red] - initial-burnt) / initial-trees   * 100</metric>
    <enumeratedValueSet variable="burn-one">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="fire-stop-grid">
      <value value="true"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="density">
      <value value="60"/>
    </enumeratedValueSet>
    <steppedValueSet variable="N" first="2" step="1" last="150"/>
    <enumeratedValueSet variable="Neighborhood">
      <value value="&quot;Moore&quot;"/>
    </enumeratedValueSet>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
default
0.0
-0.2 0 0.0 1.0
0.0 1 1.0 0.0
0.2 0 0.0 1.0
link direction
true
0
Line -7500403 true 150 150 90 180
Line -7500403 true 150 150 210 180
@#$#@#$#@
0
@#$#@#$#@
